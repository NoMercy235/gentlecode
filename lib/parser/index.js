...
const { DefineConstNode } = require('./nodes/DefineConstNode');
const { PrintCallNode } = require('./nodes/PrintCallNode');
      ...
      case 'please':
        ...
        switch (token.value) {
          case 'define':
            ...
            // TODO: find a better logic to extract tokens until the end of statement
            let extractedToken;
            let expression = [];
            while (extractedToken = parsedTokens.shift()) {
              if (extractedToken.value === ';') break;
              expression.push(extractedToken);
            }
